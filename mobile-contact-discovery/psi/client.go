...
import (
	...
	"log"
	"math"
	fbs "mobile-contact-discovery/fbs"
...
func (c *Client) ProcessPuncHintResp(hintResps *[]pir.PuncHintResp, threshold float64, numWorker int) {
...
func RunOfflineAll(offServer string, segsize int, numsegs int, npir int, threshold float64, numWorker int) *Client {
	...
	c.ProcessPuncHintResp(DecodeAllHintResp(hr, numWorker), threshold, numWorker)
	log.Printf("PSI\tProcessed Hints for %d partitions", (len(c.ClientPIR)))
	return &c
}

// TODO call C++ oprf here
// For now this function only returns random values
func (c *Client) RunOPRF_Android(exp uint16, first bool, oprf string, prfType string) {
	...
	c.CfIndices = util.Random_CFValues(pir.RandSource(), first, prfType, uint32(int(1)<<exp), c.N_cf)
}
