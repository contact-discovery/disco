...
CSocketChannel::CSocketChannel(const char *hostname, uint16_t port,
  ...
  if (isServer) {
    ...
    if (setsockopt (serversocket, SOL_SOCKET, SO_SNDTIMEO, &timeout,
                sizeof timeout) < 0)
        std::cerr << "setsockopt failed\n";
...
void CSocketChannel::send_all(uint8_t *data, size_t length) {
  ...
  while (totalsent < length) {
    ...
    if (sent < 0) {
      // TODO fix socket timeouts
      // On mobile, long running server tasks cause socket timeouts      std::cerr << "strerror(errno): " << strerror(errno) << "\n";
      throw std::runtime_error("socket sent error");
    } 
    totalsent += sent;
    counter++;
  }
