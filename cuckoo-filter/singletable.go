...
// Encode returns a byte slice representing a TableBucket
func (t *SingleTable) BucketsSlice(start uint64, amount uint64) []byte {
	// num bytes per bucket
	bucketsizeByte := t.BitsPerItem() / 8 * t.kTagsPerBucket
	end := (start + amount) * uint64(bucketsizeByte)
	if end <= uint64(len(t.bucket)) {
		return t.bucket[uint64(start)*uint64(bucketsizeByte) : end]
	}
	log.Fatalln("End value out of bucket range. end: ", end, " len(t.bucket): ", len(t.bucket))
	return nil
}

func (t *SingleTable) Buckets() []byte {
	return t.bucket
}
