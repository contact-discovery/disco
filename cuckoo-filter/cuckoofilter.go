...
type table interface {
	...
	Decode([]byte) error
	Reset()
	BucketsSlice(start uint64, end uint64) []byte
	Buckets() []byte
}
...
// NumBuckets returns the number of buckets of filter's table
func (f *Filter) NumBuckets() uint {
	return f.table.NumBuckets()
}
...
func (f *Filter) Add_sim(item []byte) (bool, uint32) {
	if f.victim.used {
		return false, 0
	}
	i, tag := f.generateIndexTagHash(item)
	return f.addImpl_sim(i, tag)
}
...
func (f *Filter) addImpl_sim(i uint, tag uint32) (bool, uint32) {
	curIndex := i
	curTag := tag
	var oldTag uint32
...
func (f *Filter) Bucket(bucketNum uint64) []byte {
	return f.table.BucketsSlice(bucketNum, 1)
}

func (f *Filter) BucketsSlice(start uint64, amount uint64) []byte {
	return f.table.BucketsSlice(start, amount)
}

func (f *Filter) Buckets() []byte {
	return f.table.Buckets()
}
